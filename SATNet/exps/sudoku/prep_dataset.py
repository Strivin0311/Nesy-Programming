...
def get_sudoku_satnet():
    root = "../../SATNet_MNIST"
    train_set = SudoKuDataset(root, split='train')
    test_set = SudoKuDataset(root, split='test')
...
class SudoKuDataset(Dataset):
    def __init__(self, root='./data', split='train', numSamples=None, randomSeed=None):
        ...
        # shape of [num, 729]
        self.x_path = ('%s/%s/board_img.pt' %(root, split))
        self.y_path = ('%s/%s/label.pt' %(root, split))
        self.z_path = ('%s/%s/input_mask.pt' %(root, split))
    ...
    def __getitem__(self, index):
        return self.x[index].cuda(), self.z[index].cuda().int(),  self.y[index].cuda()
    def __len__(self):
        return len(self.x)
...
if __name__ == "__main__":
    pass
    train_set, test_set = get_sudoku_rrn()
    data, is_input, Y = train_set[0]
    #
    #
    # data_dir = '../../sudoku'
    # train_set, test_set = get_sudoku_visual(data_dir, True, None)
    #
    # data, is_input, Y = train_set[0]  # data:shape=(81,1,28,28), is_input:shape=(729), Y:shape=(729)
    #
    def show(data, is_input, Y, id=0):
        from torchvision.utils import make_grid
        img = make_grid(data, nrow=9)
        img = img.cpu().numpy()
        plt.imshow(np.transpose(img, (1,2,0)))
        plt.savefig("input_" + str(id) + ".png")

        is_input = is_input.view(-1, 9) # shape => (81,9)
        Y = Y.view(-1, 9) # shape => (81,9)

        is_input = torch.amax(is_input, dim=1) # shape => (81)
        Y = torch.argmax(Y, dim=1) # shape => (81)

        print("{}: ".format(id))
        print(is_input)
        print(Y)

    # show(data_, is_input_, Y_, id=0)
    show(data, is_input, Y, id=1)
