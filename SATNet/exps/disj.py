...
def main():
    ...
    with open(os.path.join(args.data_dir, str(L), 'labels.pt'), 'rb') as f:
        Y = torch.load(f).float().unsqueeze(dim=1)
    ...
    t(0, model, optimizer, test_logger, test_set, args.testBatchSz)
    for epoch in range(1, args.nEpoch + 1):
        train(epoch, model, optimizer, train_logger, train_set, args.batchSz)
        t(epoch, model, optimizer, test_logger, test_set, args.testBatchSz)
...
@torch.no_grad()
def t(epoch, model, optimizer, logger, dataset, batchSz):
