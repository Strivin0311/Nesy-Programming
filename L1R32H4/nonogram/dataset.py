...
class Nonogram_Dataset(Dataset):
    def __init__(self, data_path, board_dim, max_num_per_hint, limit, seed, mask_ratio=0):
        """
        Args:
    ...
    def process_data(self, all_unsolved, all_solved, indices):
        ...
        return self.__mask_data(X, Y)

    def __mask_data(self, X, Y):
        if self.mask_ratio > 0:
            import math
            game_size = int(math.sqrt(X.shape[1]))  # 7
            mask_number = int(game_size * self.mask_ratio)  # the number of constraints to be masked
            for i in range(Y.shape[0]):
                idxs = torch.randperm(game_size)[:mask_number]
                row = torch.rand(size=(1,)).item() <= 0.5
                if row:
                    for r in idxs:
                        for j in range(game_size):
                            Y[i, r*game_size + j] = -100
                else:
                    for c in idxs:
                        for j in range(game_size):
                            Y[i, j*game_size + c] = -100
