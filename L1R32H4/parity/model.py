...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.ln_f2 = nn.LayerNorm(config.num_classes*config.block_size)
        self.head2 = nn.Linear(config.num_classes*config.block_size, config.num_classes, bias=False)
    ...
    def forward(self, idx, targets=None, idx_ulb=None):
        ...
        for _ in range(self.n_recur):
            for block in self.blocks:
                ...
                if self.all_layers and targets is not None:
                    l1 = self.head(self.ln_f(x))
                    l2 = self.head2(self.ln_f2(l1.view(l1.shape[0], -1)))
                    logits.append(l2)
        ...
        if targets is not None:
            ...
            for logit in logits:  # (batch_size, 81, 9)
                loss += F.cross_entropy(logit, targets.view(-1))
