...
class DrivingDataset(Dataset):
    ...
    def _init_transform(self):
        trans = []
        ## image augmentation
        if self.cfg.AUGMENT_BLUR:  # blur
            trans.append(transforms.GaussianBlur(kernel_size=3, sigma=(0.1, 2.0)))
        if self.cfg.AUGMENT_JITTER:  # jitter
            trans.append(transforms.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.2, hue=0.2))
        if self.cfg.AUGMENT_FLIP:  # flip
            trans.append(transforms.RandomHorizontalFlip(p=0.5))
        ## image transformation
        trans.append(transforms.Resize((self.cfg.INPUT_HEIGHT, self.cfg.INPUT_WIDTH)))  # resize
        trans.append(transforms.ToTensor())  # to tensor
        trans.append(transforms.Normalize(  # normalize
